...
class _ClientSettingsScreenState extends State<ClientSettingsScreen> {
  ...
  Map<String, dynamic>? schoolData; // School information (single school per client)
  ...
  Future<void> _loadClientSettings() async {
    ...
    try {
      final data = await _apiService.getClientSettings(currentClientData['schema_name']);
      if (data != null) {
        setState(() {
  ...
  Widget _buildContent() {
    return RefreshIndicator(
      ...
      child: SingleChildScrollView(
        physics: const AlwaysScrollableScrollPhysics(), // Ensures pull-to-refresh works even when content fits
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    gradient: const LinearGradient(
                      colors: [Color(0xFF001b3f), Color(0xFF004aad)], // Brand colors
                    ),
            ...
            _buildInfoRow('Schema Name', currentClientData['schema_name'] ?? ''),
            _buildInfoRow('Organization Type', currentClientData['organization_type'] ?? ''),
            _buildInfoRow('Account Type', currentClientData['account_type'] ?? ''),
            const SizedBox(height: 16),
            ...
            FilledButton.icon(
              ...
              style: FilledButton.styleFrom(
                ...
                foregroundColor: Colors.white, // Ensures white text in both themes
              ),
            ),
          ],
        ),
          ...
          children: [
            ...
            Wrap(
              ...
              children: [
                ...
                _buildFeatureChip('myFinancials', settingsData?['myFinancials'] == true),
                _buildFeatureChip('myAttendance', settingsData?['myAttendance'] == true),
                _buildFeatureChip('myAttendance Community', settingsData?['myAttentanceComm'] == true),
                _buildFeatureChip('myForms', settingsData?['myForms'] == true),
                _buildFeatureChip('myInventory', settingsData?['myInventory'] == true),
                _buildFeatureChip('myShop', settingsData?['myShop'] == true),
                _buildFeatureChip('myTimesheets', settingsData?['myTimesheets'] == true),
                _buildFeatureChip('myVolunteers', settingsData?['myVolunteers'] == true),
                _buildFeatureChip('myMusic', settingsData?['myDigitalLibrary'] == true),
              ],
            ),
          ],
        ),
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(
                ...
                width: value ? 1.5 : 1, // Thicker border for green to make it more prominent
              ),
            ),
            child: Text(
              value ? 'Yes' : 'No',
              style: TextStyle(
                ...
                fontSize: 13,
                fontWeight: FontWeight.w600, // Bolder font for better visibility
              ),
            ),
          ),
        ],
    ...
    return Chip(
      label: Text(
        ...
        style: TextStyle(
          ...
          fontWeight: enabled ? FontWeight.w600 : FontWeight.normal, // Bold when enabled
        ),
      ),
  ...
  void _showAdminsDialog() async {
    ...
    try {
      final admins = await _apiService.getClientAdmins(currentClientData['schema_name']);
  ...
  void _showEditClientDialog() {
    final nameController = TextEditingController(text: currentClientData['name']);
    final domainController = TextEditingController(text: currentClientData['domain']);
    final orgTypeController = TextEditingController(text: currentClientData['organization_type']);
    final accountTypeController = TextEditingController(text: currentClientData['account_type']);
                          ...
                          try {
                            ...
                            if (mounted) {
                              ...
                              if (success) {
                                ...
                                setState(() {
                                  currentClientData['name'] = nameController.text;
                                  currentClientData['domain'] = domainController.text;
                                  currentClientData['organization_type'] = orgTypeController.text;
                                  currentClientData['account_type'] = accountTypeController.text;
                                });
  ...
  void _showEditSchoolDialog() {
    ...
    final booleanFields = {
      ...
      'activate_pp_advanced_checkout': schoolData?['activate_pp_advanced_checkout'] ?? false,
    ...
    final controllers = {
      ...
      'address_state': TextEditingController(text: schoolData?['address_state']),
      'address_zipcode': TextEditingController(text: schoolData?['address_zipcode']?.toString()),
      'booster_org': TextEditingController(text: schoolData?['booster_org']),
      'contact_fullname': TextEditingController(text: schoolData?['contact_fullname']),
      'contact_phone1': TextEditingController(text: schoolData?['contact_phone1']),
      'contact_phone2': TextEditingController(text: schoolData?['contact_phone2']),
      'google_cal_link': TextEditingController(text: schoolData?['google_cal_link']),
      'invoices_reply_to_email': TextEditingController(text: schoolData?['invoices_reply_to_email']),
      'returned_email_recipients': TextEditingController(text: schoolData?['returned_email_recipients']),
      'primary_color': TextEditingController(text: schoolData?['primary_color']),
      'secondary_color': TextEditingController(text: schoolData?['secondary_color']),
      'paypal_fee_checkout': TextEditingController(text: schoolData?['paypal_fee_checkout']),
      'paypal_email_address': TextEditingController(text: schoolData?['paypal_email_address']),
      'paypal_fee_percentage': TextEditingController(text: schoolData?['paypal_fee_percentage']),
      'paypal_fixed_fee': TextEditingController(text: schoolData?['paypal_fixed_fee']),
      'paypal_myshop_fee_percentage': TextEditingController(text: schoolData?['paypal_myshop_fee_percentage']),
      'paypal_myshop_fixed_fee': TextEditingController(text: schoolData?['paypal_myshop_fixed_fee']),
      'paypal_school_initials': TextEditingController(text: schoolData?['paypal_school_initials']),
      'request_fee_support_help_text': TextEditingController(text: schoolData?['request_fee_support_help_text']),
      'request_fee_support_recipient': TextEditingController(text: schoolData?['request_fee_support_recipient']),
    };
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        TextFormField(
                          ...
                          decoration: const InputDecoration(labelText: 'Zipcode', border: OutlineInputBorder()),
                        ),
                        const SizedBox(height: 16),
                        TextFormField(
                          controller: controllers['booster_org'],
                          decoration: const InputDecoration(labelText: 'Booster Organization', border: OutlineInputBorder()),
                        ),
                        const SizedBox(height: 24),
                        ...
                        TextFormField(
                          ...
                          decoration: const InputDecoration(labelText: 'Phone 1', border: OutlineInputBorder()),
                        ),
                        const SizedBox(height: 16),
                        TextFormField(
                          controller: controllers['contact_phone2'],
                          decoration: const InputDecoration(labelText: 'Phone 2', border: OutlineInputBorder()),
                        ),
                        const SizedBox(height: 16),
                        TextFormField(
                          controller: controllers['google_cal_link'],
                          decoration: const InputDecoration(labelText: 'Google Calendar Link', border: OutlineInputBorder()),
                        ),
                        const SizedBox(height: 16),
                        TextFormField(
                          controller: controllers['invoices_reply_to_email'],
                          decoration: const InputDecoration(labelText: 'Invoice Reply-To Email', border: OutlineInputBorder()),
                        ),
                        const SizedBox(height: 16),
                        TextFormField(
                          controller: controllers['returned_email_recipients'],
                          decoration: const InputDecoration(labelText: 'Returned Email Recipients', border: OutlineInputBorder()),
                        ),
                        const SizedBox(height: 24),
                        const Text('Branding', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                        const SizedBox(height: 12),
                        Row(
                          children: [
                            Expanded(
                            ...
                            Expanded(
                              child: TextFormField(
                                ...
                                decoration: const InputDecoration(labelText: 'Secondary Color', border: OutlineInputBorder()),
                              ),
                            ),
                          ],
                        ),
                        const SizedBox(height: 24),
                        const Text('PayPal Settings', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                        const SizedBox(height: 12),
                        TextFormField(
                        ...
                        TextFormField(
                          ...
                          decoration: const InputDecoration(labelText: 'PayPal Fee Checkout', border: OutlineInputBorder()),
                        ),
                        const SizedBox(height: 16),
                        Row(
                          children: [
                            Expanded(
                              child: TextFormField(
                                controller: controllers['paypal_fee_percentage'],
                            ...
                            Expanded(
                              child: TextFormField(
                                ...
                                decoration: const InputDecoration(labelText: 'Fixed Fee', border: OutlineInputBorder()),
                              ),
                            ),
                          ],
                        ),
                        ...
                        TextFormField(
                          ...
                          decoration: const InputDecoration(labelText: 'School Initials', border: OutlineInputBorder()),
                        ),
                        const SizedBox(height: 24),
                        const Text('Account Options', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                        const SizedBox(height: 12),
                        StatefulBuilder(
                          builder: (context, setState) => Column(
                            children: [
                              SwitchListTile(
                                ...
                                onChanged: (value) => setState(() => booleanFields['can_split'] = value),
                              ),
                              SwitchListTile(
                                title: const Text('Can Unsplit'),
                                value: booleanFields['can_unsplit']!,
                                onChanged: (value) => setState(() => booleanFields['can_unsplit'] = value),
                              ),
                              SwitchListTile(
                                title: const Text('Advanced Checkout Active'),
                                value: booleanFields['activate_pp_advanced_checkout']!,
                                onChanged: (value) => setState(() => booleanFields['activate_pp_advanced_checkout'] = value),
                              ),
                              SwitchListTile(
                                title: const Text('Account Credits'),
                                value: booleanFields['account_credits']!,
                                onChanged: (value) => setState(() => booleanFields['account_credits'] = value),
                              ),
                              SwitchListTile(
                                title: const Text('Enable Account Credits'),
                                value: booleanFields['enable_account_credits']!,
                                onChanged: (value) => setState(() => booleanFields['enable_account_credits'] = value),
                              ),
                              SwitchListTile(
                                title: const Text('Password Reset'),
                                value: booleanFields['password_reset']!,
                                onChanged: (value) => setState(() => booleanFields['password_reset'] = value),
                              ),
                              SwitchListTile(
                                title: const Text('Registration'),
                                value: booleanFields['registration']!,
                                onChanged: (value) => setState(() => booleanFields['registration'] = value),
                              ),
                              SwitchListTile(
                                title: const Text('Student ID'),
                                value: booleanFields['student_id']!,
                                onChanged: (value) => setState(() => booleanFields['student_id'] = value),
                              ),
                              SwitchListTile(
                                title: const Text('Allergies'),
                                value: booleanFields['allergies']!,
                                onChanged: (value) => setState(() => booleanFields['allergies'] = value),
                              ),
                              SwitchListTile(
                                title: const Text('Food Allergies'),
                                value: booleanFields['food_allergies']!,
                                onChanged: (value) => setState(() => booleanFields['food_allergies'] = value),
                              ),
                              SwitchListTile(
                                title: const Text('OTC Medicines'),
                                value: booleanFields['otc_medicines']!,
                                onChanged: (value) => setState(() => booleanFields['otc_medicines'] = value),
                              ),
                              SwitchListTile(
                                title: const Text('Gender'),
                                value: booleanFields['gender']!,
                                onChanged: (value) => setState(() => booleanFields['gender'] = value),
                              ),
                              SwitchListTile(
                                title: const Text('Bus Number'),
                                value: booleanFields['bus_number']!,
                                onChanged: (value) => setState(() => booleanFields['bus_number'] = value),
                              ),
                            ],
                          ),
                        ),
  ...
  void _showEditAppSettingsDialog() {
    ...
    final booleanFields = {
      ...
      'integrated_pp_merchant': settingsData?['integrated_pp_merchant'] ?? false,
      ...
      'myAttendance': settingsData?['myAttendance'] ?? false,
      'myAttentanceComm': settingsData?['myAttentanceComm'] ?? false,
      'myForms': settingsData?['myForms'] ?? false,
      'myInventory': settingsData?['myInventory'] ?? false,
      'myShop': settingsData?['myShop'] ?? false,
      'myTimesheets': settingsData?['myTimesheets'] ?? false,
    ...
    final controllers = {
      ...
      'subscription_type': TextEditingController(text: settingsData?['subscription_type']),
      'renew_cycle': TextEditingController(text: settingsData?['renew_cycle']),
      'annual_price': TextEditingController(text: settingsData?['annual_price']),
      'max_users': TextEditingController(text: settingsData?['max_users']?.toString()),
      'max_emails_per_day': TextEditingController(text: settingsData?['max_emails_per_day']?.toString()),
      'preferred_payment': TextEditingController(text: settingsData?['preferred_payment']),
      'commission_percentage': TextEditingController(text: settingsData?['commission_percentage']),
      'mybb_client_id': TextEditingController(text: settingsData?['mybb_client_id']),
      'myFiles_max_size': TextEditingController(text: settingsData?['myFiles_max_size']),
    };
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        TextFormField(
                          ...
                          decoration: const InputDecoration(labelText: 'App Type', border: OutlineInputBorder()),
                        ),
                        const SizedBox(height: 16),
                        TextFormField(
                        ...
                        const SizedBox(height: 24),
                        const Text('Subscription Details', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                        const SizedBox(height: 12),
                        TextFormField(
                          controller: controllers['renew_cycle'],
                          decoration: const InputDecoration(labelText: 'Renew Cycle', border: OutlineInputBorder()),
                        ),
                        const SizedBox(height: 16),
                        TextFormField(
                          controller: controllers['annual_price'],
                          decoration: const InputDecoration(labelText: 'Annual Price', border: OutlineInputBorder(), prefixText: '\$'),
                          ...
                          children: [
                            Expanded(
                              child: TextFormField(
                                ...
                                decoration: const InputDecoration(labelText: 'Max Users', border: OutlineInputBorder()),
                                keyboardType: TextInputType.number,
                              ),
                            ),
                            const SizedBox(width: 16),
                            Expanded(
                              child: TextFormField(
                                ...
                                decoration: const InputDecoration(labelText: 'Max Emails/Day', border: OutlineInputBorder()),
                                keyboardType: TextInputType.number,
                              ),
                            ),
                          ],
                        ...
                        TextFormField(
                          ...
                          decoration: const InputDecoration(labelText: 'Preferred Payment', border: OutlineInputBorder()),
                        ),
                        const SizedBox(height: 16),
                        TextFormField(
                        ...
                        const SizedBox(height: 24),
                        const Text('MyBlueBoard API', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                        const SizedBox(height: 12),
                        TextFormField(
                        ...
                        TextFormField(
                          ...
                          decoration: const InputDecoration(labelText: 'Files Max Size', border: OutlineInputBorder()),
                        ),
                        const SizedBox(height: 24),
                        const Text('Status Flags', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                        const SizedBox(height: 12),
                        StatefulBuilder(
                          builder: (context, setState) => Column(
                            children: [
                              SwitchListTile(
                                ...
                                onChanged: (value) => setState(() => booleanFields['is_trial'] = value),
                              ),
                              SwitchListTile(
                                title: const Text('Block Emails'),
                                value: booleanFields['block_emails']!,
                                onChanged: (value) => setState(() => booleanFields['block_emails'] = value),
                              ),
                              SwitchListTile(
                                title: const Text('Suspended'),
                                value: booleanFields['suspended']!,
                                onChanged: (value) => setState(() => booleanFields['suspended'] = value),
                              ),
                              SwitchListTile(
                                title: const Text('Locked'),
                                value: booleanFields['locked']!,
                                onChanged: (value) => setState(() => booleanFields['locked'] = value),
                              ),
                              SwitchListTile(
                                title: const Text('Exception'),
                                value: booleanFields['exception']!,
                                onChanged: (value) => setState(() => booleanFields['exception'] = value),
                              ),
                              SwitchListTile(
                                title: const Text('Integrated PayPal Merchant'),
                                value: booleanFields['integrated_pp_merchant']!,
                                onChanged: (value) => setState(() => booleanFields['integrated_pp_merchant'] = value),
                              ),
                            ],
                          ),
                        ),
                        const SizedBox(height: 24),
                        const Text('Enabled Modules', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                        const SizedBox(height: 12),
                        StatefulBuilder(
                          builder: (context, setState) => Column(
                            children: [
                              SwitchListTile(
                                ...
                                onChanged: (value) => setState(() => booleanFields['myEmail'] = value),
                              ),
                              SwitchListTile(
                                title: const Text('myBoard Module'),
                                value: booleanFields['myBoard']!,
                                onChanged: (value) => setState(() => booleanFields['myBoard'] = value),
                              ),
                              SwitchListTile(
                                title: const Text('myFiles Module'),
                                value: booleanFields['myFiles']!,
                                onChanged: (value) => setState(() => booleanFields['myFiles'] = value),
                              ),
                              SwitchListTile(
                                title: const Text('myFinancials Module'),
                                value: booleanFields['myFinancials']!,
                                onChanged: (value) => setState(() => booleanFields['myFinancials'] = value),
                              ),
                              SwitchListTile(
                                title: const Text('myAttendance Module'),
                                value: booleanFields['myAttendance']!,
                                onChanged: (value) => setState(() => booleanFields['myAttendance'] = value),
                              ),
                              SwitchListTile(
                                title: const Text('myAttendance Community Module'),
                                value: booleanFields['myAttentanceComm']!,
                                onChanged: (value) => setState(() => booleanFields['myAttentanceComm'] = value),
                              ),
                              SwitchListTile(
                                title: const Text('myForms Module'),
                                value: booleanFields['myForms']!,
                                onChanged: (value) => setState(() => booleanFields['myForms'] = value),
                              ),
                              SwitchListTile(
                                title: const Text('myInventory Module'),
                                value: booleanFields['myInventory']!,
                                onChanged: (value) => setState(() => booleanFields['myInventory'] = value),
                              ),
                              SwitchListTile(
                                title: const Text('Shop Module'),
                                value: booleanFields['myShop']!,
                                onChanged: (value) => setState(() => booleanFields['myShop'] = value),
                              ),
                              SwitchListTile(
                                title: const Text('myTimesheets Module'),
                                value: booleanFields['myTimesheets']!,
                                onChanged: (value) => setState(() => booleanFields['myTimesheets'] = value),
                              ),
                              SwitchListTile(
                                title: const Text('myVolunteers Module'),
                                value: booleanFields['myVolunteers']!,
                                onChanged: (value) => setState(() => booleanFields['myVolunteers'] = value),
                              ),
                              SwitchListTile(
                                title: const Text('myMusic Module'),
                                value: booleanFields['myDigitalLibrary']!,
                                onChanged: (value) => setState(() => booleanFields['myDigitalLibrary'] = value),
                              ),
                            ],
                          ),
                        ),
                          ...
                          try {
                            ...
                            if (mounted) {
                              ...
                              if (success) {
                                ...
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(
                                    content: Text('Failed to update app settings'),
                                    backgroundColor: Colors.red,
                                  ),
                                );
                              }
